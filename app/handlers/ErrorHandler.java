...
public class ErrorHandler extends DefaultHttpErrorHandler {
    ...
    private Logger.ALogger logger = Logger.of("components.handlers.ErrorHandler");
    ...
    public CompletionStage<Result> onClientError(RequestHeader request, int statusCode, String message) {
        return CompletableFuture.completedFuture(Results.status(statusCode , jsonError("clienterror", message, request.method() + " " + request.uri())));
    }
    ...
    public CompletionStage<Result> onClientErrorCompletionStage(RequestHeader request, int statusCode, String errorCode, String message) {
        return CompletableFuture.completedFuture(Results.status(statusCode , jsonError(errorCode, message, request.method() + " " + request.uri())));
    }
    ...
    public Result onServerError(String errorCode, Throwable e, String description) {
        ...
        logger.error("[" + contextId + "] onServerError " + errorCode + " - " + description, e);
    ...
    public CompletionStage<Result> onDevServerError(RequestHeader request, UsefulException exception) {
        long contextId = Http.Context.current().id();
        logger.error("[" + contextId + "] onDevError internalservererror - " + exception.title + " " + exception.description, exception);
    ...
    public CompletionStage<Result> onProdServerError(RequestHeader request, UsefulException exception) {
        long contextId = Http.Context.current().id();
        logger.error("[" + contextId + "] onProdError internalservererror - " + exception.title + " " + exception.description, exception);
        return CompletableFuture.completedFuture(Results.internalServerError(jsonError("internalservererror", exception.title, "uri" + request.uri() + " " + exception.description)));
    }

    private JsonNode jsonError(String code, String message, String description) {
        return Json.newObject().put("code", code).put("message", message).put("description", description);
    }
}